...
# ---- read in the  matrix ----
expMat <- ReadMtx(mtx="./matrixes_run1/BCP-ALL-FEB22_DBEC_MolsPerCell_MEX/matrix.mtx",
                  cells="./matrixes_run1/BCP-ALL-FEB22_DBEC_MolsPerCell_MEX/barcodes.tsv",
                  features="./matrixes_run1/BCP-ALL-FEB22_DBEC_MolsPerCell_MEX/features.tsv") 
...
# ---- remove CD3 column, since was not used in this run ----
ab <- ab %>% select(-"CD3:SK7|CD3E|AHS0033|pAbO")
rna <- expMat %>% dplyr::select(!ends_with("pAbO")) 
...
# ---- add cell assignment ----
cell_assign <- fread(file = "./matrixes_run1/BCP-ALL-FEB22_Sample_Tag_Calls.csv",
                     sep = ",", header = T) %>%
...
png(file="./qc_plots_run1/n_cells_x_sample.png",
...
seuratObj <- subset(seuratObj ,
                    subset = Sample_Name %notin% c("Multiplet", "Undetermined"))

# ---- verify number of cells for each sample ----
png(file="./qc_plots_run1/n_cells_x_sample_no_multiplets_undet.png",
    height = 800, width = 1000, units = "px", res = 150)
seuratObj@meta.data %>%
  ggplot(aes(x = Sample_Name, fill = Sample_Name)) +
  geom_bar() +
...
png(file="./qc_plots_run1/umi_per_cell.png",
    height = 800, width = 1000, units = "px", res = 150)
seuratObj@meta.data %>%
...
png(file="./qc_plots_run1/genes_per_cell.png",
    height = 800, width = 1000, units = "px", res = 150)
seuratObj@meta.data %>%
...
png(file="./qc_plots_run1/mito_perc_per_cell.png",
    height = 800, width = 1000, units = "px", res = 150)
seuratObj@meta.data %>%
...
# ---- vlnplots for recap ----
png(file="./qc_plots_run1/vlnplot_recap.png",
...
png(file="./qc_plots_run1/complexity.png",
    height = 800, width = 1000, units = "px", res = 150)
seuratObj@meta.data %>%
...
png(file="./qc_plots_run1/qc_metrics_combo.png",
    height = 800, width = 1000, units = "px", res = 150)
seuratObj@meta.data %>%
...
saveRDS(seuratObj,
        file = "./seurat_objects_run1/bcpall_run1_seurat_unfiltered_step1.rds")
...
final_filtered.seuratObj <- CreateSeuratObject(filtered_counts,
                                               meta.data = filtered.seuratObj@meta.data)

############################# re-assess QC metrics #############################

# ---- verify number of cells for each sample ----
png(file="./qc_plots_run1/n_cells_x_sample_filtered.png",
    height = 800, width = 1000, units = "px", res = 150)
...
png(file="./qc_plots_run1/umi_per_cell_filtered.png",
    height = 800, width = 1000, units = "px", res = 150)
final_filtered.seuratObj@meta.data %>%
  ggplot(aes(color = Sample_Name, x = nCount_RNA, fill = Sample_Name)) +
  geom_density(alpha = 0.2) +
...
# ---- visualize the number of genes per cell ----
# (valid cell at least 300 genes)
png(file="./qc_plots_run1/genes_per_cell_filtered.png",
    height = 800, width = 1000, units = "px", res = 150)
final_filtered.seuratObj@meta.data %>%
  ggplot(aes(color = Sample_Name, x = nFeature_RNA, fill = Sample_Name)) +
  geom_density(alpha = 0.2) +
...
saveRDS(final_filtered.seuratObj,
        file = "./seurat_objects_run1/bcpall_run1_seurat_filtered_with_ab_step1.rds")

saveRDS(ab, file = "./seurat_objects_run1/bcpall_run1_ab_step_1.rds")
