...
DimHeatmap(dx_d15_blasts_pt13, dims = 1:20, cells = 500, balanced = TRUE)
...
predictions_pt13 <- SingleR(test=dx_d15_blasts_pt13@assays$RNA$counts, assay.type.test=1, 
                            ref=ref.data, labels=ref.data$label.main, de.method = 'classic')

table(predictions_pt13$labels)

dx_d15_blasts_pt13$ann_celltype <- predictions_pt13$labels

dx_d15_blasts_pt13_ann_joined <- JoinLayers(dx_d15_blasts_pt13)
dx_d15_blasts_pt13_ann.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt13_ann_joined)
...
DimPlot(dx_d15_blasts_pt13, reduction = "umap", group.by = "ann_celltype",
...
png(file="./merge_plots_dx_d15_blasts_couples_annotation/umap_pt13_annotation.png", 
    height = 2600, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt13_ann.cds, color_cells_by = "ann_celltype",
...
# our data
dx_d15_blasts_pt13 <- JoinLayers(dx_d15_blasts_pt13)
abseq <- GetAssay(dx_d15_blasts_pt13, "AB")
...
dx_d15_blasts_pt13 <- map_Query(
  exp_query = dx_d15_blasts_pt13[["RNA"]]$counts, 
  metadata_query = dx_d15_blasts_pt13@meta.data,
  ref_obj = BM_ref,
  vars = batchvar
)
...
dx_d15_blasts_pt13_joined <- JoinLayers(subset(dx_d15_blasts_pt13, mapping_error_QC == 'Pass'))
dx_d15_blasts_pt13.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt13_joined)

png(file="./merge_plots_dx_d15_blasts_couples_projection_labels/umap_pt13_projection_labels.png", height = 2700, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt13.cds, color_cells_by = "predicted_CellType_BoneMarrowMap_Broad",
                     label_cell_groups = F,  cell_size = 1.5) + 
  scale_color_manual(values = proj_colors) +
  nice_theme2 +
  geom_segment(aes(x=-11, y=-6, xend=-9, yend=-6), arrow = arrow(length=unit(.4, 'cm')), color = "black") + #x
  geom_segment(aes(x=-11, y=-6, xend=-11, yend=-4.65), arrow = arrow(length=unit(.4, 'cm')), color = "black") + #y
  labs(x = "UMAP1", y = "UMAP2") +
  guides(color = guide_legend(ncol=4, override.aes = list(size = 5)))
dev.off()
