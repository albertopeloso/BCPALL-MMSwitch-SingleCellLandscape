
setwd('/media/data/lab/CLL1_sc_final_analyses_2024/')

library(tidyverse)
library(Seurat)
library(data.table)
library(findPC)
library(harmony)

set.seed(1998)

# ---- load filtered object ----
dx_d15_combined <- readRDS("./seurat_objects_merge_dx_d15/bcp_all_merge_dx_d15_seurat_filtered_with_ab_dimred.rds")

# ---- add metadata ----
md <- dx_d15_combined@meta.data %>% as.data.frame
md <- md %>%
  mutate(Switch = case_when(
    Sample_Name == "PT06_DX" ~ "sw+",
    Sample_Name == "PT07_DX" ~ "sw+",
    Sample_Name == "PT08_DX" ~ "sw-",
    Sample_Name == "PT13_DX" ~ "sw+",
    Sample_Name == "PT14_DX" ~ "sw+",
    Sample_Name == "PT15_DX" ~ "sw+",
    Sample_Name == "PT06_D15" ~ "sw+",
    Sample_Name == "PT07_D15" ~ "sw+",
    Sample_Name == "PT08_D15" ~ "sw-",
    Sample_Name == "PT13_D15" ~ "sw+",
    Sample_Name == "PT14_D15" ~ "sw+",
    Sample_Name == "PT15_D15" ~ "sw+"
  )) %>%
  mutate(Timepoint = case_when(
    grepl("DX", Sample_Name) ~ "DX",
    grepl("D15", Sample_Name) ~ "D15",
  ))

dx_d15_combined <- AddMetaData(dx_d15_combined, metadata = md)

# ---- check precomputer dimred ----
DimPlot(dx_d15_combined, label = T)
DimPlot(dx_d15_combined, group.by = "Sample_Name",label = T)


# remove T-cell cluster 3,5,6,21,11
# remove B-cell cluster 10,14,17,19
dx_d15_blasts <- subset(dx_d15_combined,
                        idents = c(3,5,6,21,11,10,14,17,19,22),
                        invert = T)

# ---- add details of cells from PT15-cl8 ----
cl8_cells <- read.table("./palantir_on_couples_202501/cl8_cells.txt",
                        header = T)
DimPlot(dx_d15_combined, cells.highlight = as.list(cl8_cells))

# ---- remove healthy Myelo cells of cluster 8 PT15 ----
dx_d15_blasts[["CellName"]] <- colnames(dx_d15_blasts)
dx_d15_blasts <- subset(dx_d15_blasts, cells = cl8_cells$x,
                        invert = T)

DefaultAssay(dx_d15_blasts) <- "RNA"

# log normalization of UMI counts
dx_d15_blasts <- NormalizeData(dx_d15_blasts,
                               normalization.method = "LogNormalize")

# ---- PCA ----
dx_d15_blasts <- FindVariableFeatures(dx_d15_blasts,
                                  selection.method = "vst",
                                  nfeatures = 2000)

# scale the data
dx_d15_blasts <- ScaleData(dx_d15_blasts, features = rownames(dx_d15_blasts))

# perform PCA
dx_d15_blasts <- RunPCA(dx_d15_blasts,npcs = 60)

# first UMAP, with random dims
DefaultAssay(dx_d15_blasts) = "RNA"
dx_d15_blasts <- RunUMAP(dx_d15_blasts, dims = 1:46)
dx_d15_blasts <- FindNeighbors(dx_d15_blasts, dims = 1:46)
dx_d15_blasts <- FindClusters(dx_d15_blasts, resolution = 0.5)

DimPlot(dx_d15_blasts, group.by = "Sample_Name")
DimPlot(dx_d15_blasts, group.by = "orig.ident")
DimPlot(dx_d15_blasts, label = T)

# ---- set a theme for plotting ----
nice_theme2 <- theme(plot.title=element_text(face = "bold",
                                             family = "Helvetica Light",
                                             size=14),
                     axis.text.y=element_blank(),
                     axis.text.x=element_blank(),
                     axis.line.x = element_blank(),
                     axis.line.y = element_blank(),
                     axis.ticks = element_blank(),
                     axis.title.x = element_text(hjust = 0.0475, vjust = 5.5,
                                                 face = "bold", family = "Karla",
                                                 size = 16),
                     axis.title.y = element_text(hjust = 0.05, vjust = -5,
                                                 angle=90, face = "bold",
                                                 family = "Karla", size = 16),
                     legend.text = element_text(family = "Karla",size = 16),
                     legend.position = "bottom",
                     # legend.justification.right = "bottom",
                     legend.title = element_blank())

# ---- plot using monocle ----
dx_d15_blasts_joined <- JoinLayers(dx_d15_blasts)
dx_d15_blasts.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_joined)

# color switch
png(file="./merge_plots_dx_d15_blasts/umap_split_switch_monocle_blasts_202507.png",
    height = 2500, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts.cds, color_cells_by = "Switch",
                     label_cell_groups = F, cell_size = 0.5) + 
  nice_theme2 +
  scale_color_manual(values =  c("sw-"="#46acc8", "sw+"="#fa9c32"),
                     labels = c("mmSWneg", "mmSWpos")) + 
  geom_segment(aes(x=-14.5, y=-16, xend=-11.8, yend=-16),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") + 
  geom_segment(aes(x=-14.5, y=-16, xend=-14.5, yend=-12.8),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") +
  labs(x = "UMAP1", y = "UMAP2")
dev.off()

# color sample 
png(file="./merge_plots_dx_d15_blasts/umap_split_sample_monocle_blasts_202507.png",
    height = 2500, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts.cds, color_cells_by = "Sample_Name",
                     label_cell_groups = F, cell_size = 0.5) + 
  nice_theme2 +
  geom_segment(aes(x=-14.5, y=-16, xend=-11.8, yend=-16),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") + 
  geom_segment(aes(x=-14.5, y=-16, xend=-14.5, yend=-12.8),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") +
  labs(x = "UMAP1", y = "UMAP2")
dev.off()

# color timepoint
png(file="./merge_plots_dx_d15_blasts/umap_color_pts_dx_d15_color_timepoint_monocle_blasts_202507.png",
    height = 2500, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts.cds, color_cells_by = "Timepoint",
                     label_cell_groups = F, cell_size = 0.5) + 
  nice_theme2 +
  scale_color_manual(values = c("DX" = "#9999ff", "D15" = "#0000e5"),
                     labels = c("DX" = "Diagnosis", "D15" = "Day+15"),
                     breaks = c("DX", "D15")) +
  geom_segment(aes(x=-14.5, y=-16, xend=-11.8, yend=-16),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") + 
  geom_segment(aes(x=-14.5, y=-16, xend=-14.5, yend=-12.8),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") +
  labs(x = "UMAP1", y = "UMAP2")
dev.off()

# color patient and timepoint
png(file="./merge_plots_dx_d15_blasts/umap_color_patients_monocle_blasts_202507.png",
    height = 2700, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts.cds, color_cells_by = "Sample_Name",
                     label_cell_groups = F, cell_size = 0.5) + 
  nice_theme2 +
  scale_color_manual(values = c("PT06_DX" = "#d992eb", "PT06_D15" = "#AC92EB",
                                "PT07_DX" = "#4FC1E8", "PT07_D15" = "#4f75e8",
                                "PT08_DX" = "#94b76c", "PT08_D15" = "#607f3e",
                                "PT13_DX" = "#FFCE54", "PT13_D15" = "#d39700",
                                "PT14_DX" = "#ED5564", "PT14_D15" = "#c71627",
                                "PT15_DX" = "#ED9255", "PT15_D15" = "#985726"),
                     labels = c("PT06_DX" = "BCPALL#06_DX", "PT06_D15" = "BCPALL#06_D15",
                                "PT07_DX" = "BCPALL#07_DX", "PT07_D15" = "BCPALL#07_D15",
                                "PT08_DX" = "BCPALL#08_DX", "PT08_D15" = "BCPALL#08_D15",
                                "PT13_DX" = "BCPALL#09_DX", "PT13_D15" = "BCPALL#09_D15",
                                "PT14_DX" = "BCPALL#10_DX", "PT14_D15" = "BCPALL#10_D15",
                                "PT15_DX" = "BCPALL#11_DX", "PT15_D15" = "BCPALL#11_D15")) +
  
  geom_segment(aes(x=-14.5, y=-16, xend=-11.8, yend=-16),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") + 
  geom_segment(aes(x=-14.5, y=-16, xend=-14.5, yend=-12.8),
               arrow = arrow(length=unit(.4, 'cm')), color = "black") +
  labs(x = "UMAP1", y = "UMAP2")
dev.off()
