...
# update 202507 - use filtered object, without 'normal' myeloid cluster
...
dx_d15_blasts_pt15 <- ScaleData(dx_d15_blasts_pt15, features = rownames(dx_d15_blasts_pt15))

dx_d15_blasts_pt15 <- RunPCA(dx_d15_blasts_pt15,npcs = 60)

ElbowPlot(dx_d15_blasts_pt15, ndims = 50)
DimHeatmap(dx_d15_blasts_pt15, dims = 1:20, cells = 500, balanced = TRUE)
...
predictions_pt15 <- SingleR(test=dx_d15_blasts_pt15@assays$RNA$counts,
                            assay.type.test=1, 
                            ref=ref.data, labels=ref.data$label.main,
                            de.method = 'classic')

table(predictions_pt15$labels)

dx_d15_blasts_pt15$ann_celltype <- predictions_pt15$labels

dx_d15_blasts_pt15_ann_joined <- JoinLayers(dx_d15_blasts_pt15)
dx_d15_blasts_pt15_ann.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt15_ann_joined)
...
DimPlot(dx_d15_blasts_pt15, reduction = "umap", group.by = "ann_celltype", 
...
png(file="./merge_plots_dx_d15_blasts_couples_annotation/umap_pt15_annotation_202507.png", 
    height = 2600, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt15_ann.cds, color_cells_by = "ann_celltype",
                     label_cell_groups = F,  cell_size = 1.5) + 
  scale_color_manual(values = cell_colors, labels = labels) +
  nice_theme2 +
  geom_segment(aes(x=-14.5, y=-8, xend=-12, yend=-8), 
               arrow = arrow(length=unit(.4, 'cm')), color = "black") + #x
  geom_segment(aes(x=-14.5, y=-8, xend=-14.5, yend=-6), 
...
# our data
dx_d15_blasts_pt15 <- JoinLayers(dx_d15_blasts_pt15)
abseq <- GetAssay(dx_d15_blasts_pt15, "AB")

# MAPPING #
batchvar <- c('Sample_Name')

# Map query dataset using Symphony (Kang et al 2021)
dx_d15_blasts_pt15 <- map_Query(
  exp_query = dx_d15_blasts_pt15[["RNA"]]$counts, 
  metadata_query = dx_d15_blasts_pt15@meta.data,
  ref_obj = BM_ref,
  vars = batchvar
)

donor_key <- "Sample_Name" 
dx_d15_blasts_pt15 <- dx_d15_blasts_pt15 %>% 
  calculate_MappingError(., reference = BM_ref, MAD_threshold = 3,
                         threshold_by_donor = TRUE, donor_key = donor_key) 

# Get QC Plots
QC_plots <- plot_MappingErrorQC(dx_d15_blasts_pt15)

# CELL TYPE ASSIGNMENT #

# Predict Hematopoietic Cell Types by KNN classification 
dx_d15_blasts_pt15 <- predict_CellTypes(
  query_obj = dx_d15_blasts_pt15, 
  ref_obj = BM_ref, 
  initial_label = 'initial_CellType_BoneMarrowMap', 
  final_label = 'predicted_CellType_BoneMarrowMap'
) 

DimPlot(subset(dx_d15_blasts_pt15, mapping_error_QC == 'Pass'), 
...
dx_d15_blasts_pt15_joined <- JoinLayers(subset(dx_d15_blasts_pt15,
                                               mapping_error_QC == 'Pass'))
dx_d15_blasts_pt15.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt15_joined)

png(file="./merge_plots_dx_d15_blasts_couples_projection_labels/umap_pt15_projection_labels_202507.png",
    height = 2700, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt15.cds, 
