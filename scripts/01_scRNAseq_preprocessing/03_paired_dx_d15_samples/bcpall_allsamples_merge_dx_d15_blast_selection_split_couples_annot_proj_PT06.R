...
predictions_pt06 <- SingleR(test=dx_d15_blasts_pt06@assays$RNA$counts, assay.type.test=1, 
                       ref=ref.data, labels=ref.data$label.main, de.method = 'classic')

table(predictions_pt06$labels)

dx_d15_blasts_pt06$ann_celltype <- predictions_pt06$labels

dx_d15_blasts_pt06_ann_joined <- JoinLayers(dx_d15_blasts_pt06)
dx_d15_blasts_pt06_ann.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt06_ann_joined)
...
DimPlot(dx_d15_blasts_pt06, reduction = "umap", group.by = "ann_celltype", 
...
png(file="./merge_plots_dx_d15_blasts_couples_annotation/umap_pt06_annotation.png",
    height = 2800, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt06_ann.cds, color_cells_by = "ann_celltype",
                     label_cell_groups = F,  cell_size = 1.5) + 
  scale_color_manual(values = cell_colors, labels = labels) +
  nice_theme2 +
  geom_segment(aes(x=-6, y=-11, xend=-4.1, yend=-11), arrow = arrow(length=unit(.4, 'cm')), color = "black") + #x
  geom_segment(aes(x=-6, y=-11, xend=-6, yend=-9), arrow = arrow(length=unit(.4, 'cm')), color = "black") + #y
  labs(x = "UMAP1", y = "UMAP2") +
...
# our data
dx_d15_blasts_pt06 <- JoinLayers(dx_d15_blasts_pt06)
abseq <- GetAssay(dx_d15_blasts_pt06, "AB")
...
dx_d15_blasts_pt06 <- map_Query(
  exp_query = dx_d15_blasts_pt06[["RNA"]]$counts, 
  metadata_query = dx_d15_blasts_pt06@meta.data,
  ref_obj = BM_ref,
  vars = batchvar
)

donor_key <- "Sample_Name"
dx_d15_blasts_pt06 <- dx_d15_blasts_pt06 %>% calculate_MappingError(., reference = BM_ref, MAD_threshold = 3, 
                                          threshold_by_donor = TRUE, donor_key = donor_key) 

QC_plots <- plot_MappingErrorQC(dx_d15_blasts_pt06)

# CELL TYPE ASSIGNMENT #

# Predict Hematopoietic Cell Types by KNN classification 
dx_d15_blasts_pt06 <- predict_CellTypes(
  query_obj = dx_d15_blasts_pt06, 
  ref_obj = BM_ref, 
  initial_label = 'initial_CellType_BoneMarrowMap', 
  final_label = 'predicted_CellType_BoneMarrowMap' 
) 

DimPlot(subset(dx_d15_blasts_pt06, mapping_error_QC == 'Pass'), 
...
nice_theme2 <- theme(plot.title=element_text(face = "bold", 
                                             family = "Helvetica Light", size=14),
                     axis.text.y=element_blank(),
                     axis.text.x=element_blank(),
                     axis.line.x = element_blank(),
                     axis.line.y = element_blank(),
...
dx_d15_blasts_pt06_joined <- JoinLayers(subset(dx_d15_blasts_pt06, 
                                               mapping_error_QC == 'Pass'))
dx_d15_blasts_pt06.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt06_joined)

png(file="./merge_plots_dx_d15_blasts_couples_projection_labels/umap_pt06_projection_labels.png",
    height = 2800, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt06.cds, 
