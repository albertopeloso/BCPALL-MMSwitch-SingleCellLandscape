...
DimHeatmap(dx_d15_blasts_pt07, dims = 1:20, cells = 500, balanced = TRUE)
...
dx_d15_blasts_pt07 <- FindClusters(dx_d15_blasts_pt07, resolution = 1.5)

DimPlot(dx_d15_blasts_pt07, group.by = "Sample_Name")
DimPlot(dx_d15_blasts_pt07, pt.size = 2)
...
predictions_pt07 <- SingleR(test=dx_d15_blasts_pt07@assays$RNA$counts,
                            assay.type.test=1, 
                            ref=ref.data, labels=ref.data$label.main, 
                            de.method = 'classic')

table(predictions_pt07$labels)

dx_d15_blasts_pt07$ann_celltype <- predictions_pt07$labels

dx_d15_blasts_pt07_ann_joined <- JoinLayers(dx_d15_blasts_pt07)
dx_d15_blasts_pt07_ann.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt07_ann_joined)
...
DimPlot(dx_d15_blasts_pt07, reduction = "umap", group.by = "ann_celltype", 
...
png(file="./merge_plots_dx_d15_blasts_couples_annotation/umap_pt07_annotation.png", height = 2800, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt07_ann.cds, color_cells_by = "ann_celltype",
                     label_cell_groups = F,  cell_size = 1.5) + 
  scale_color_manual(values = cell_colors, labels = labels) +
  nice_theme2 +
  geom_segment(aes(x=-8.5, y=-11, xend=-6.5, yend=-11), arrow = arrow(length=unit(.4, 'cm')), color = "black") + #x
  geom_segment(aes(x=-8.5, y=-11, xend=-8.5, yend=-9.1), arrow = arrow(length=unit(.4, 'cm')), color = "black") + #y
  labs(x = "UMAP1", y = "UMAP2") +
...
# our data
dx_d15_blasts_pt07 <- JoinLayers(dx_d15_blasts_pt07)
abseq <- GetAssay(dx_d15_blasts_pt07, "AB")
...
dx_d15_blasts_pt07 <- map_Query(
  exp_query = dx_d15_blasts_pt07[["RNA"]]$counts, 
  metadata_query = dx_d15_blasts_pt07@meta.data,
  ref_obj = BM_ref,
  vars = batchvar
)

donor_key <- "Sample_Name" 
dx_d15_blasts_pt07 <- dx_d15_blasts_pt07 %>% 
  calculate_MappingError(., reference = BM_ref, MAD_threshold = 3, 
                         threshold_by_donor = TRUE, donor_key = donor_key)

# Get QC Plots
QC_plots <- plot_MappingErrorQC(dx_d15_blasts_pt07)

# CELL TYPE ASSIGNMENT #

# Predict Hematopoietic Cell Types by KNN classification 
dx_d15_blasts_pt07 <- predict_CellTypes(
  query_obj = dx_d15_blasts_pt07, 
  ref_obj = BM_ref, 
  initial_label = 'initial_CellType_BoneMarrowMap', 
  final_label = 'predicted_CellType_BoneMarrowMap'  
) 

DimPlot(subset(dx_d15_blasts_pt07, mapping_error_QC == 'Pass'),
        reduction = 'umap_projected', group.by = c('predicted_CellType_BoneMarrowMap'), 
        raster=FALSE, label=T, label.size = 4,
        split.by = "Sample_Name", ncol = 4) + theme(legend.position = "none") +
...
dx_d15_blasts_pt07_joined <- JoinLayers(subset(dx_d15_blasts_pt07, mapping_error_QC == 'Pass'))
dx_d15_blasts_pt07.cds <- SeuratWrappers::as.cell_data_set(dx_d15_blasts_pt07_joined)

png(file="./merge_plots_dx_d15_blasts_couples_projection_labels/umap_pt07_projection_labels.png",
    height = 2800, width = 2500, units = "px", res = 300)
monocle3::plot_cells(dx_d15_blasts_pt07.cds, color_cells_by = "predicted_CellType_BoneMarrowMap_Broad",
                     label_cell_groups = F,  cell_size = 1.5) + 
